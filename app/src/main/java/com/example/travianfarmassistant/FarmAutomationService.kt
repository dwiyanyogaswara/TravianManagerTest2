...
class FarmAutomationService : Service() {
    ...
    /**
     * Refresh Village dijadwalkan 1 menit setelah countdown/Next Run dimulai.
     * Refresh berjalan di luar cycle aktif agar data resource sudah terbaru saat
     * countdown berakhir dan cycle berikutnya dimulai.
     */
    ...
    private fun startAutomation() {
        ...
        // Siklus pertama: refresh village dulu agar Resource Builder tidak memakai
        // data lama. Setelah refresh selesai, siklus pertama langsung dimulai.
        initialCyclePending = true
        countdownCyclePending = true
    ...
    private fun triggerScheduledCycle() {
        ...
        // Jangan menghapus state Refresh Village di sini. Refresh sudah dijadwalkan
        // 1 menit setelah countdown dimulai dan harus selesai sebelum Resource Builder.
        getSharedPreferences(PREFS, MODE_PRIVATE).edit()
    ...
    private fun startResourceBuilderCycle() {
        ...
        // Village Data adalah single source of truth; refresh pada awal siklus sudah
        // memperbarui LinkResource dan MinLvl dari halaman game terbaru.
        val now = System.currentTimeMillis()
        if (farmListCycleStartedAt > 0L) {
            val farmDuration = (now - farmListCycleStartedAt).coerceAtLeast(0L)
            getSharedPreferences(PREFS, MODE_PRIVATE).edit()
    ...
    private fun scheduleNextRandomRun() {
        ...
        handler.removeCallbacks(nextRunRunnable)
        handler.postDelayed(nextRunRunnable, delay)
        // Setiap countdown berikutnya selalu diikuti Refresh Village tepat 1 menit
        // setelah countdown dimulai (atau segera bila interval <= 1 menit).
        scheduleVillageRefreshForNextRun(nextAt)
        logEvent("Countdown dimulai: $chosenMinutes menit; Next Run=${timeFormat.format(Date(nextAt))}")
